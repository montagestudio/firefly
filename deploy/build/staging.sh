...
if [[ -e /etc/init/firefly-login.conf ]]; then
    ...
    sudo sed -i.bak 's/export FIREFLY_PROJECT_URL=.*/export FIREFLY_PROJECT_URL="http:\/\/staging-project.montagestudio.net"/' /etc/init/firefly-login.conf
...
if [[ -e /etc/init/firefly-project.conf ]]; then
    ...
    sudo sed -i.bak 's/export FIREFLY_PROJECT_URL=.*/export FIREFLY_PROJECT_URL="http:\/\/staging-project.montagestudio.net"/' /etc/init/firefly-project.conf
